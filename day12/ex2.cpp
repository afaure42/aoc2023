...
int main(int argc, char *argv[])
{
	...
	while(std::getline(in_file, line, '\n'))
	{
		...
		std::vector<int> temp_vect = groups;
		for(int i = 0; i < 4; i++)
		{
			line += '?' + temp_line;
			groups.insert(groups.end(), temp_vect.begin(), temp_vect.end());
		}
		...
		//launch a thread to solve a line
		// promises.push_back(std::async(std::launch::async, proxy_try_solve_4, line, groups, 0, 0));

		// while ( promises.size() >= 6)
		// {
		// 	for(size_t i = 0; i < promises.size(); i++)
		// 	{
		// 		if (promises[i].wait_for(std::chrono::milliseconds(5)) == std::future_status::ready)
		// 		{
		// 			total += promises[i].get();
